...
$sqlcheck = "Select count(*) AS count from parkingRules where lotID=:id";
$data = ParkingDatabase::getDataFromSQL($sqlcheck, $params);
$count = intval($data[0]["count"]);

if($count > 0){
    echo json_encode (["message"=>"Lot ID is used in parkingRule table."]);
    exit;
}

$sql = "Delete from parkingLots WHERE lotID=:id;";
ParkingDatabase::executeSQL($sql, $params);
$message = ["message"=>"Lot Deleted Successfully"];
