...
$sqlcheck = "Select count(*) AS count from parkingRules where timeID=:id";
$data = ParkingDatabase::getDataFromSQL($sqlcheck, $params);
$count = intval($data[0]["count"]);

if($count > 0){
    echo json_encode (["message"=>"Time ID is used in parkingRule table."]);
    exit;
}

$sql = "Delete from parkingTimes WHERE timeID=:id;";
ParkingDatabase::executeSQL($sql, $params);
$message = ["message"=>"Time Deleted Successfully"];
